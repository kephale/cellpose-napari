...
def widget_wrapper():
    ...
    def run_cellpose(
        ...
        if model_match_threshold == 0.0:
            flow_threshold = 0.0
            logger.debug(
                "flow_threshold=0 => no masks thrown out due to model mismatch"
            )
        logger.debug(
            f"computing masks with cellprob_threshold={cellprob_threshold}, flow_threshold={flow_threshold}"
        )
        if model_type == "custom":
            CP = models.CellposeModel(pretrained_model=custom_model, gpu=True)
        else:
            CP = models.CellposeModel(model_type=model_type, gpu=True)
        masks, flows_orig, _ = CP.eval(
            image,
            channels=channels,
    ...
    def compute_masks(
        ...
        logger.debug(
            f"computing masks with cellprob_threshold={cellprob_threshold}, flow_threshold={flow_threshold}, mask_threshold={mask_threshold}"
        )
        maski = get_masks(
            flows_orig[3].copy(),
            iscell=(flows_orig[2] > cellprob_threshold),
            flows=flows_orig[1],
            threshold=flow_threshold * (masks_orig.ndim < 3),
        )
    ...
    @magicgui(
        ...
        main_channel=dict(
            ...
            choices=main_channel_choices,
            value=0,
            tooltip="choose channel with cells",
        ),
        optional_nuclear_channel=dict(
            widget_type="ComboBox",
            label="optional nuclear channel",
            choices=optional_nuclear_channel_choices,
            value=0,
        ...
        model_match_threshold=dict(
            widget_type="FloatSlider",
            name="model_match_threshold",
            value=27.0,
            min=0.0,
            max=30.0,
            step=0.2,
        ...
        compute_masks_button=dict(
            widget_type="PushButton",
            text="recompute last masks with new cellprob + model match",
            enabled=False,
        ),
        net_average=dict(
        ...
        resample_dynamics=dict(
            widget_type="CheckBox",
            text="resample dynamics",
            value=False,
        ...
        process_3D=dict(
            widget_type="CheckBox",
            text="process stack as 3D",
            value=False,
            tooltip="use default 3D processing where flows in X, Y, and Z are computed and dynamics run in 3D to create masks",
        ),
        stitch_threshold_3D=dict(
            widget_type="LineEdit",
            label="stitch threshold slices",
            value=0,
            tooltip='across time or Z, stitch together masks with IoU threshold of "stitch threshold" to create 3D segmentation',
        ),
        clear_previous_segmentations=dict(
            widget_type="CheckBox", text="clear previous results", value=True
        ),
        output_flows=dict(
            widget_type="CheckBox", text="output flows and cellprob", value=True
        ),
        output_outlines=dict(
            widget_type="CheckBox", text="output outlines", value=True
        ),
        omni=dict(
            widget_type="CheckBox",
            text="omnipose",
            value=False,
            tooltip="use omnipose extension for thin or intertwined shapes",
        ),
        cluster=dict(
            widget_type="CheckBox",
            text="cluster",
            value=False,
            tooltip="use DBSCAN clustering to automatically segment ROIs",
        ),
        mask_threshold=dict(
            widget_type="FloatSlider",
            name="mask_threshold",
            ...
            max=6.0,
            step=0.5,
            tooltip="threshold on mask pixel values (set higher to get more masks)",
        ),
    )
    def widget(
        viewer: Viewer,
        ...
        def _new_layers(masks, flows_orig):
            ...
            cellprob = resize_image(
                flows_orig[2], masks.shape[-2], masks.shape[-1], no_channels=True
            )
            cellprob = cellprob.squeeze()
            outlines = masks_to_outlines(masks) * masks
            if masks.ndim == 3 and widget.n_channels > 0:
                masks = np.repeat(
                    np.expand_dims(masks, axis=widget.channel_axis),
                    widget.n_channels,
                    axis=widget.channel_axis,
                )
                outlines = np.repeat(
                    np.expand_dims(outlines, axis=widget.channel_axis),
                    widget.n_channels,
                    axis=widget.channel_axis,
                )
                flows = np.repeat(
                    np.expand_dims(flows, axis=widget.channel_axis),
                    widget.n_channels,
                    axis=widget.channel_axis,
                )
                cellprob = np.repeat(
                    np.expand_dims(cellprob, axis=widget.channel_axis),
                    widget.n_channels,
                    axis=widget.channel_axis,
                )
            ...
            if widget.output_flows.value:
                layers.append(
                    viewer.add_image(
                        flows,
                        name=image_layer.name + "_cp_flows" + widget.iseg,
                        visible=False,
                        rgb=True,
                ...
                layers.append(
                    viewer.add_image(
                        cellprob,
                        name=image_layer.name + "_cp_cellprob" + widget.iseg,
                        visible=False,
                        scale=physical_scale,
                    )
                )
            if widget.output_outlines.value:
                layers.append(
                    viewer.add_labels(
                        outlines,
                        name=image_layer.name + "_cp_outlines" + widget.iseg,
                        visible=False,
                        scale=physical_scale,
                    )
                )
            layers.append(
                viewer.add_labels(
                    masks,
                    name=image_layer.name + "_cp_masks" + widget.iseg,
                    visible=False,
                    scale=physical_scale,
                )
            )
        ...
        def _new_segmentation(segmentation):
            ...
            try:
                if (
                    image_layer.ndim > 2
                    and not process_3D
                    and not float(stitch_threshold_3D)
                ):
        ...
        cp_worker = run_cellpose(
            ...
            do_3D=(
                process_3D and float(stitch_threshold_3D) == 0 and image_layer.ndim > 2
            ),
            stitch_threshold=(
                float(stitch_threshold_3D) if image_layer.ndim > 2 else 0.0
            ),
            omni=omni,
            cluster=cluster,
            mask_threshold=mask_threshold,
    ...
    def update_masks(masks):
        ...
        widget.viewer.value.layers[
            widget.image_layer.value.name + "_cp_masks" + widget.iseg
        ].data = masks
    ...
    def _compute_diameter(e: Any):
        ...
        if widget.model_type.value == "custom":
            ...
            channels = (
                [
                    max(0, widget.main_channel.value),
                    max(0, widget.optional_nuclear_channel.value),
                ],
            )
